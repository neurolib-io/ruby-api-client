...
require 'neurolib_api_client/errors'
Gem.find_files('neurolib_api_client/api/*.rb').each { |path| require path }

module NeurolibApiClient
  ##
  # Client for sending requests to Neurolib recommender system
  class NeurolibClient
    include HTTParty

    BATCH_MAX_SIZE = 10000
    USER_AGENT = {'User-Agent' => 'neurolib-ruby-api-client/3.2.0'}

    ##
    #   - +account+ -> Name of your account at Neurolib
    #   - +token+ -> Secret token obtained from Neurolib for signing requests
    ...
    def initialize(account, token, protocol = 'https', options = {})
      ...
      @token = token
      @protocol = protocol
      @base_uri = ENV['NEUROLIB_API_URI'] if ENV.key? 'NEUROLIB_API_URI'
      @base_uri||=  options[:base_uri]
      @base_uri||= 'api.neurolib.io'
    end

    ##
    #   - +request+ -> ApiRequest to be sent to Neurolib recommender
    def send(request)
